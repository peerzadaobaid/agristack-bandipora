...
def index():
    ...
    if mode == "pending" and completion_available:
        # Decorate village rows with expected_date, days_remaining, submission_pending, required_daily_avg
        import math
        for row in views.get("village_rows", []):
            ...
            if iso_date:
                ...
                row["days_remaining"] = None
            # Submission Pending = Total Survey Nos − Submitted (never negative)
            row["submission_pending"] = max(0, int(row.get("total", 0)) - int(row.get("submitted", 0)))
            # Required Daily Avg = submission_pending / days_remaining
            #   None      → no date set (show —)
            #   -1        → overdue (show "Overdue")
            #   0         → done (nothing pending, show 0)
            #   integer   → surveys per day (rounded up)
            dr = row["days_remaining"]
            sp = row["submission_pending"]
            if dr is None:
                row["required_daily_avg"] = None
            elif sp == 0:
                row["required_daily_avg"] = 0
            elif dr < 0:
                row["required_daily_avg"] = -1  # sentinel for overdue
            elif dr == 0:
...
def _resolve_download_context(pending_mode=False):
    ...
    bucket_summary_rows = []
    if pending_mode and has_completion_feature():
        # Decorate village rows with expected_date + days_remaining + submission fields
...
def _generate_workbook(views, to_date, from_date, view_key="all", pending_mode=False,
    ...
    if _include("village"):
        ...
        if pending_mode:
            # New order: S.No | Tehsil | Village | Patwari | Total | Submitted | Sub Pending | Req Daily | Verified | Approved
            headers = ["S.NO", "TEHSIL", "VILLAGE", "NAME OF PATWARI", "TOTAL SURVEY NOS",
                       "SUBMITTED", "SUBMISSION PENDING", "REQUIRED DAILY AVG",
                       "VERIFIED", "APPROVED"]
        else:
        ...
        additions_col = None
        if additions_hdr:
            additions_col = len(headers) - (2 if pending_mode else 0)
        date_col = None
        days_col = None
        if pending_mode:
            date_col = len(headers) - 1
            days_col = len(headers)
        total_cols = len(headers)
        for i, row in enumerate(views["village_rows"], start=1):
            r = i + 1
            if pending_mode:
                # Required Daily Avg formatted text
                rda = row.get("required_daily_avg")
                if rda is None:
                    rda_val = "—"
                elif rda == -1:
                    rda_val = "Overdue"
                    ...
                    rda_val = int(rda)
                vals = [i, row["tehsil"], row["village"], row["patwari"], row["total"],
                        row["submitted"], row.get("submission_pending", 0), rda_val,
                        row["verified"], row["approved"]]
                aligns = [center, left, left, left, center, center, center, center, center, center]
            else:
            ...
            for ci, (v, a) in enumerate(zip(vals, aligns), start=1):
                ...
                # Numeric formatting
                if pending_mode:
                    if ci in (5, 6, 7, 9, 10) and isinstance(v, int):
                        c.number_format = "#,##0"
                    if ci == 8 and isinstance(v, int):
                        c.number_format = "#,##0"
                else:
                    if ci in (4, 6, 7, 8) and isinstance(v, int):
                        c.number_format = "#,##0"
                if additions_col and ci == additions_col and isinstance(v, int):
                    c.number_format = "+#,##0;-#,##0;0"
            apply_band_fill(ws, r, total_cols, row.get("band"))
        ...
        # TOTAL row: use column 3 if pending (village is col 3), else col 2
        tot_label_col = 3 if pending_mode else 2
        ws.cell(row=vt, column=tot_label_col, value=("TOTAL (Filtered)" if use_filtered else ("TOTAL (Pending)" if pending_mode else "TOTAL"))).alignment = left
        if pending_mode:
            # Grand totals in new column positions
            total_sum = sum(r.get("total", 0) for r in views["village_rows"])
            submitted_sum = sum(r.get("submitted", 0) for r in views["village_rows"])
            pending_sum = sum(r.get("submission_pending", 0) for r in views["village_rows"])
            ov = views["overall_totals"]
            ws.cell(row=vt, column=5, value=total_sum).alignment = center
            ws.cell(row=vt, column=6, value=submitted_sum).alignment = center
            ws.cell(row=vt, column=7, value=pending_sum).alignment = center
            ws.cell(row=vt, column=8, value="").alignment = center  # RDA total is meaningless
            ws.cell(row=vt, column=9, value=ov["verified"]).alignment = center
            ws.cell(row=vt, column=10, value=ov["approved"]).alignment = center
            if additions_col:
        ...
        for c in range(1, total_cols + 1):
            ...
            cc.font = tot_font; cc.fill = tot_fill; cc.border = border
            if pending_mode:
                if c in (5, 6, 7, 9, 10): cc.number_format = "#,##0"
            else:
                if c in (4, 6, 7, 8): cc.number_format = "#,##0"
            if additions_col and c == additions_col and isinstance(cc.value, int):
                cc.number_format = "+#,##0;-#,##0;0"
        if pending_mode:
            widths = [7, 14, 24, 24, 18, 14, 18, 18, 14, 14]
        else:
